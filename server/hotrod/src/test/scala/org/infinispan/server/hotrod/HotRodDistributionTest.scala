...
class HotRodDistributionTest extends HotRodMultiNodeTest {
   ...
   def testDistributedPutWithTopologyChanges(m: Method) {
      ...
      assertHashTopologyReceived(resp.topologyResponse.get, servers, cacheName, 2, 1)
      ...
      assertStatus(resp, Success)
      assertHashTopologyReceived(resp.topologyResponse.get, servers, cacheName)
      ...
      try {
         ...
         assertHashTopologyReceived(hashTopologyResp, allServers, cacheName)
      ...
      assertHashTopologyReceived(hashTopologyResp, servers, cacheName)
