...
object HotRodTestingUtil extends Log {
   ...
   def assertHashTopologyReceived(topoResp: AbstractTopologyResponse, servers: List[HotRodServer],
                                  cacheName: String) {
   ...
   def assertHashIds(hashIds: Map[ServerAddress, Seq[Int]], servers: List[HotRodServer], cacheName: String) {
      val consistentHash = servers.head.getCacheManager.getCache(cacheName).getAdvancedCache
      ...
      assert(hashIds.size == numSegments * numOwners)
