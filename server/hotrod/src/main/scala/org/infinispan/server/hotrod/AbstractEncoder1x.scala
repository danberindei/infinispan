   ...
   protected def writeHashTopologyHeader(topoRsp: AbstractTopologyResponse,
      ...
      topoRsp match {
         case h: AbstractHashDistAwareResponse => {
            ...
            val totalNumServers = numSegments * h.numOwners
            ...
            val segmentHashIds = ch.asInstanceOf[DefaultConsistentHash].getSegmentHashes
            for (segmentIdx <- 0 until numSegments) {
               val segmentHashId = segmentHashIds.get(segmentIdx)
               val segmentOwners = ch.locateOwnersForSegment(segmentIdx)
               for (ownerIdx <- 0 until h.numOwners) {
                  val address = members.get(segmentOwners.get(ownerIdx % segmentOwners.size))
                  val hashId = segmentHashId + ownerIdx
                  writeString(address.host, buffer)
                  writeUnsignedShort(address.port, buffer)
